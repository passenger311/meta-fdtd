...
for k = 500:999
   ...
   sumparaperp = tmp{2}/(2*pi/(k*1d-9))^2;
   para = tmp{7}/(2*pi/(k*1d-9))^2;
   perp = tmp{4}/(2*pi/(k*1d-9))^2;

   fprintf(fidsave,'%i      %e\n', k,sqrt(sumparaperp(sizemax)/2));
   fprintf(fidsave2,'%i      %e\n', k,sqrt(sumparaperp(1)/2));
   fprintf(fidsave3,'%i      %e\n', k,sumparaperp(sizemax)*(2*pi/(k*1d-9))^2/pi/2);
   fprintf(fidsave4,'%i      %e\n', k,sqrt(sumparaperp(1)*(2*pi/(k*1d-9))^2/pi/2));
end
fclose(fidsave);
fclose(fidsave2);
fclose(fidsave3);
