...
real.pump.freq 	=	real.pump.omega/(2*math.pi)
real.pump.lambda 	=	c_SI/real.pump.freq

real.probe.freq 	=	c_SI/real.probe.lambda
real.probe.omega 	=	real.probe.freq*2*math.pi
...
real.wire.num 	=	10 	-- Number of wire
real.wire.h 		=	0.1
real.wire.w 		=	0.1
real.wire.s		=	0.1
real.wire.xst 	=	1
...
real.S.lambda 	= 	real.pump.lambda					-- Permitivity of surrounding material
...
for loop=1, real.wire.num, 1 do
        a = string.format("%s%d","wire",loop)
...
a = string.format("filling_wire")
b = string.format("wire")
cfg:CREATE_GEO{
          a,
          scene=scene_filling,
          grid=grid[b],
