...
real_cyl_length = 800.0		-- length of cylinder
real_hmetal_sub = 100.0		-- thickness of substrate
real_dist = 200			-- distance of top, sides end end of wire
real_wavelength = 380.0		-- real wavelength
...
resolution = real_wavelength/1 -- resolution of wavelength in optically thickest medium (even number)
...
hdist_tfsf_i = cyl_radius+1.5*dist 
hdist_tfsf_jn = hmetal_sub
hdist_tfsf_jp = cyl_diameter + hspacer + dist
...
probe_on_injy = true
pinv_wl = conv/380
pampl = 1.0075029765377e26 --1e7   -- electric field strength in V/m
...
--ncyc_probe_start = 10000--8*16384 --math.floor((attackl+sustainl+decayl)*resolution/dt+.5)
...
elseif (mat == 'silver2') then
  ...
  real_omegaDL = 15600/2/pi--13900/2/pi -- plasma frequency in THz 
...
--foutput:write(conv/375,"\n")
--foutput2:write(375, "\n")
--foutput:write(conv/385,"\n")
--foutput2:write(385, "\n")
--[[
i=876
foutput:write(conv/i,"\n")
