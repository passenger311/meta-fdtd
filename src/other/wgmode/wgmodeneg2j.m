function [ Ex,Ey,Hx,Hy,range] = wgmodeneg2j( epsfile, lambdainv, hx, hy, neffguess )
...
% --- padding leps and lmu from both sides
...
leps = [leps(:,:);leps(sl,:)];
leps = [leps(1,:);leps(:,:)];
%lmu(sl+1:sl+1,1) = lmu(sl,1);
%lmu(sl+1:sl+1,2) = lmu(sl,2);
%lmu(sl+1:sl+1,3) = lmu(sl,3);
lmu = [lmu(:,:);lmu(sl,:)];
lmu = [lmu(1,:);lmu(:,:)];
...
for i=1:si+2
   ...
   mu(i) = lmu(l,3);
   if sqrt(eps(i).*mu(i)) > nmax 
     nmax = sqrt(eps(i).*mu(i));
   end
end
...
%  4 3
    ...
    for i=1:si
        l=i+1;   
        % --- block 1 (Hx-Hx)
        C(1,i) = lmu(l-1,1)/lmu(l-1,3)./(hx*hx);
        D(1,i) = -2./(hx*hx) + lmu(l,1)*leps(l,2)*om*om;
        E(1,i) = lmu(l+1,1)/lmu(l,3)./(hx*hx);
        % --- block 2 (Hy-Hy)
        C(2,i) = leps(l,1)/leps(l,3)/(hx*hx);
        D(2,i) = lmu(l,2)*leps(l,1)*om*om - leps(l,1)/(hx*hx)*(1./leps(l+1,3)+1./leps(l,3));
        E(2,i) = leps(l,1)/leps(l+1,3)/(hx*hx);    
    end
...
Ey = zeros(1,si);
    ...
    for i=1:si-1
        l = i+1;
        Hz(i) = -1/keff(1,1)/lmu(l,3)*( (lmu(l+1,1)*Hx(i+1)-lmu(l,1)*Hx(i))/hx ); % = Hz / i
    end
