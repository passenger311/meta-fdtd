...
d_pmlx=180;	% Thickness of PMLs in x direction
d_pmly=50;	% Thickness of PMLs in y direction
strength = 0.3;	% Strength of PMLs
...
if(exist('mode_neff.dat')~=2)
  ...
  fprintf(f_out,'#lambda\tRe neff\tIm neff\tProp. length (micron)\n');
  fclose(f_out);
end
f_out=fopen('mode_neff.dat','a');
fprintf(f_out,'%3.0f\t%3.2f\t%5.4f\t%5.4f\t%f\n',2*cyl_rad,conv/lambdainv,real(neff),imag(neff),conv/lambdainv/2/pi/imag(neff)/1000);
fclose(f_out);
...
if(exist('mode_full.dat')~=2)
  ...
  fprintf(f_out,'#Radius\tlambda\tRe neff\tIm neff\talpha\tConf\tA_0\tA_En2\tA_Emax\tn_Eav\tgamma\talpha_g_bulk\n');
  fclose(f_out);
...
fprintf(f_out,'%3.0f\t%3.2f\t%5.4f\t%5.4f\t%6.0f\t%4.3f\t%6.0f\t%6.0f\t%6.0f\t%6.2f\t%6.2f\t%6.0f\n',2*cyl_rad,conv/lambdainv,real(neff),imag(neff),alpha,Gamma,A_eff0,A_eff_En2,A_eff_Emax,n_Eav,gamma/1e12,alpha2*2.3/n_Eav/Gamma);
fclose(f_out);
...
filename=sprintf('mode_%3.0f_%3.0f_%5.3f+i%5.3f.png',2*cyl_rad,conv/lambdainv,real(neff),imag(neff));
