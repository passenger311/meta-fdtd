...
Obtain the free OOFILE classes used in this test from 
...
/**
Testing method just to write out to the console
*/
void
simpleData::dumpData()
{
	...
	printf(" Age=%d", mAge);
	if (mIsProgrammer)
		puts(" Is a programmer\n");
	else
		puts(" Is NOT a programmer\n");
}
...
*/
void 
sdParser::startElement(const char* name, const char** /*atts*/)
{
...
void 
sdParser::endElement(const char*)
{
	mState = eNone;
}
...
int main()
{
  ...
  if (strlen(filename)==0)
  	return 0;  // was break from loop reading files 
...
// read back thefile
  ...
  puts("sample data after reading back\n");
  theData.dumpData();
