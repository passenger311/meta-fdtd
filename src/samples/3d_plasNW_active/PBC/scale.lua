...
real_cyl_radius = rad/2--55.0 --radius of silver layers
real_hspacer = 10.0 --radius of spacer layer
real_cyl_length = len--290.0 --y dimension of perforation
real_hmetal_sub = 130.0 --z dimension radius of metal substrate
real_dist = 800 --distance of measuring plane from structure
real_disth = 300
...
cyl_length = math.floor(real_cyl_length /conv+.5)
...
hdist_tfsf_j = math.floor(cyl_length/2) --2*cyl_diameter
hdist_ntff_jmin = math.floor(cyl_length/2)
hdist_ntff_jmax = cyl_length - math.floor(cyl_length/2)
hdist_tfsf_kn = hmetal_sub
hdist_ntff_kn = hdist_tfsf_kn
hdist_tfsf_kp = cyl_diameter + hspacer + disth
...
print("ntff-domain y dir (grid):                 ", hdist_ntff_jmax)
