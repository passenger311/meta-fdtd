...
real_hwidthx = 1600.0      -- half width of capacitor plates
real_hwidthy = 800.0
...
while i <= num_np do --true do
  tmp_rnp = 20
  tmp_xnp = math.random(-real_hwidthx + 2*tmp_rnp,real_hwidthx - 2*tmp_rnp)
  tmp_ynp = math.random(-real_hwidthy + 2*tmp_rnp,real_hwidthy - 2*tmp_rnp)
  tmp_znp = math.random(-real_hdist + 1.5*tmp_rnp,real_hdist - 1.5*tmp_rnp)
  k=0; l=l+1
  for j = 1,i-1 do
