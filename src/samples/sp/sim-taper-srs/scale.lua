...
resolution = 60              -- resolution of wavelength in optically thickest medium (even number)         
dt         = 0.570           -- time step
...
if tm then               -- tm mode: Ey dominant
...
real_width_edw    = 0.4                 -- evanescent decay width

real_length_i1      = 1.0                 -- length in 1
real_length_i2      = 0.5                 -- 0.5length in 2
real_length_it       = 1.5                 -- 2.5length in taper
real_length_c       = 1.0                 -- 2.0length channel 
real_length_ot      = 1.5                 -- 2.0length out taper
real_length_o2     = 0.5                 -- 0.5length out 2
real_length_o1     = 1.0                 -- 1.0length out 1
real_width_i        = 0.4                 -- width in 
real_width_c        = 0.05                 -- 0.400width channel
real_width_o        = 0.4                 -- width out 
real_width_ii       = infty               -- width clad in 
real_width_cc      = infty               -- width clad channel/center
real_width_oo      = infty                -- 0.2width clad out 0.180
real_width_gi       = 0.00                -- 0.05width sio2 seperation gap input
real_width_go      = 0.00                -- 0.05width sio2 seperation gap output
real_width_gc      = 0.00                -- 0.05width sio2 seperation gap center
...
pulse0 = {  -- pulse in periods T of excitation
	width=10,
        offset=0,
        attack=25,
        sustain=0,
...
for k,v in pairs(pulse0) do 
  pulse[k] = round(v*resolution*nref_max)
end


--- calculate time steps 

rt_cyc = ( 2 * length * nref_si ) / dt
pulse_cyc = pulse.offset + pulse.attack + pulse.sustain + pulse.decay

ncyc        = round(2 * rt_cyc + pulse_cyc)       -- number of time steps

--- define amplitudes 

ampl_s = 1/100;    -- signal in units of 1/sqrt(chi3R)
ampl_p = 50/100;   -- pump in units of 1/sqrt(chi3R)
...
ici = round (hwidth_i + width_gi + width_edw ) 
icc = round (hwidth_c + width_gc + width_edw ) 
ico = round (hwidth_o + width_go + width_edw ) 
