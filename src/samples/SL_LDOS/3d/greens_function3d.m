...
resolution=10;
DT=0.574;
ncyc_max=60000;%2*(32767+1);%(65535+1)*(10/resolution)/4; % this number can be divided by a power of 2 to reduce the number of spectrum points;
%ncyc_max=40000;
nstep=1;%1*(10/resolution);
natt=12021+1;%3606*(0.574/DT)*(10/resolution)+1;
nhwhm=721;%120*(0.574/DT)*(10/resolution);
l_inv=0.0066666666666667;%0.013333333333333/(10/resolution);
...
j=1e6.*cos(om.*(ncyc-natt)*DT).*exp(-g.^2.*((ncyc-natt)*DT).^2);
j_full=1e6.*cos(om.*(ncyc_full-natt)*DT).*exp(-g.^2.*((ncyc_full-natt)*DT).^2);
%plot(ncyc,j);break;
...
%plot(abs(p_om))
[tmp, min_f]=min(abs(abs(p_om)-max(abs(p_om))./8));
[tmp, pmax_f]=max(abs(p_om));
if min_f<pmax_f
  [tmp, max_f]=min(abs(abs(p_om(min_f+10:size(p_om)))-max(abs(p_om))./8));
  max_f=min_f+10+max_f;
else
  max_f=min_f;
  [tmp, min_f]=min(abs(abs(p_om(1:max_f-10))-max(abs(p_om))./8));
end
...
g_real=g*(1e9/resolution)^3;
%g2_real=g2*(1e9/resolution)^3;
%g3_real=g3*(1e9/resolution)^3;
g_theo=4.*pi^2./3.*freq.^3*(1e9/resolution)^3;

figure(1);
subplot(2,2,1);
plot(freq_real, g_real(:,1),'o', freq_real, g_theo, 'red', 'LineWidth', 1.5)
xlabel('Frequency [THz]','FontSize',15);
ylabel('Greens function [1/m^3]','FontSize',15);
set(gca,'FontSize',15);
...
print(gcf, '-dpng', '-r300', 'greens_function3d.png');
...
fid1=fopen('greens_function3d.dat','w');
...
set(gca,'FontSize',15);
print(gcf, '-dpng', '-r300', 'enhancement3d.png');

fid1=fopen('enhancement3d.dat','w');
