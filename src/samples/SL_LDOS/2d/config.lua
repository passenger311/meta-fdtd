...
real.t.s 	= 	1.0e-12 		-- Pulse sustain time (s)
...
ncycles 	=	4*(65535+1)-1--round(sim_t/real.dt) 	-- Number of cycles
ncyc_meas	=	400
...
cfg:SRC{
   HARDJ{
      invlambda = dx/real.flvl.wE,
      amplitude = 1,
      pulse = {
         shape="Gaussian",
         width=330,--math.floor(pwidthl*resolution/dt+.5),
         offset=0,--math.floor(poffsetl*resolution/dt+.5),
         attack=9000,--math.floor(pattackl*resolution/dt+.5),
         sustain=0,--math.floor(psustainl*resolution/dt+.5),
         decay=120000,--math.floor(pdecayl*resolution/dt+.5)
      },
   },
   REG{
      POINT{
         { c_width, g_yst+taskid, 0, ":", 1, 0, 0}
      }
   },
}
