...
cfg:FDTD{
   ...
   OUT{
      file = { "VTK", "E_xz" },
      type = { "E", "N" },
      time = { math.floor(ncycles/10), 3*math.floor(ncycles/10), 100 },
      REG{
         BOX{
            { -hdist_tfsf_i+1, hdist_tfsf_i-1, 4, 0, 0, 1, -hdist_tfsf_k+1, hdist_tfsf_k-1, 4 }
         }
      },
--      on = false
   },
   ...
   OUT{
      ...
      REG{
         BOX{
            { 0, 0, 1, -hdist_tfsf_j+1, hdist_tfsf_j-1, 4, -hdist_tfsf_k+1, hdist_tfsf_k-1, 4 }
         }
      },
--      on = false
   },
...
cfg:SRC{
   TFSFBOX{
      invlambda = 1.2*inv_wavelength,
      amplitude = 3e8,
      pulse = {
         shape="Gaussian",
         width=math.floor(pwidthl*resolution*n_max+.5),
         offset=math.floor(poffsetl*resolution*n_max+.5),
         attack=math.floor(pattackl*resolution*n_max+.5),
         sustain=math.floor(psustainl*resolution*n_max+.5),
         decay=math.floor(pdecayl*resolution*n_max+.5)
      },
      planewave = { phi=0.0, theta=0.0+5*(taskid-1), psi=0.0, nrefr=nrefr }
   },
   REG{
...
cfg:MAT{
   THREELVL{
      invlambda = { .2*inv_wavelength, 1.2*inv_wavelength, inv_wavelength },
      gamma = { inv_wavelength/100, inv_wavelength/100, inv_wavelength/100 },
      sigma = { 0, 0, 0 },
      mx = { { 0,0 }, { 0.1,0 }, { 0.1,0 } },
      my = { { 0,0 }, { 0.1,0 }, { 0.1,0 } },
      mz = { { 0,0 }, { 0.1,0 }, { 0.1,0 } },
   ...
   OUT{
      ...
      REG{
         POINT{
            { 0, 0, -math.floor(rnp[1]-rshell[1]/2) }
         }
      },
      on = true
   },
