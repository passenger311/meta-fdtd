!-*- F90 -*------------------------------------------------------------
!
!  module: matlvfourlvlep / meta
!
!  Effective Maxwell Bloch material module for isotropic 4 level system with langevin noise terms.
!  This variation replaces the optical pump transition with a constant rate.
!
!  subs:
!
!    InitializeMatlvfourlvlep
!    FinalizeMatlvfourlvlep
!    ReadMatlvfourlvlepObj
!    StepEMatlvfourlvlep
!    StepHMatlvfourlvlep
!    SumJEKHMatlvfourlvlep
!
!----------------------------------------------------------------------
...
! =====================================================================
!
! The Matlvfourlvlep module calculates the reponse of an isotropic effective 4 level bloch system with langevin noise terms
!
...
! 
!
! StepHMatlvfourlvlep: update eq. Pa_i(n+1) = c1a * Pa_i(n) + c2a * Pa_i(n-1) + c3a * E_i(n) * ( N1(n) - N2(n) ) +Noise
!                  update eq. Pb_i(n+1) = c1b * Pb_i(n) + c2b * Pb_i(n-1) + c3b * E_i(n) * ( N0(n) - N3(n) ) + Noise
!                and update N (see below)
! StepEMatlvfourlvlep: update eq. E_i(n+1) = E_i(n+1)* - epsinv * (Pa_i(n+1) - Pa_i(n) + Pb_i(n+1) - Pb_i(n))
!
...
! The resulting equations are calculated in StepHMatlvfourlvlep in two steps. The first bit depends on E(n+1) and must be 
...
module matlvfourlvlep
  ...
  M4_MATHEAD_DECL({MATLVFOURLVLEP},MAXMATOBJ,{
...
!----------------------------------------------------------------------
  ...
  subroutine ReadMatlvfourlvlepObj(funit,lcount)

    M4_MODREAD_DECL({MATLVFOURLVLEP}, funit,lcount,mat,reg,out)
    real(kind=8) :: v(2)
    real(kind=8) :: c(3)
    real(kind=8) :: d(4)
    logical :: eof,err
    ...
    M4_WRITE_DBG(". enter ReadMatlvfourlvlepObj")

    M4_MODREAD_EXPR({MATLVFOURLVLEP},funit,lcount,mat,reg,3,out,{ 
    ...
    mat%gamma10 = d(4)
     ...
     call readfloat(funit,lcount,mat%rp) !pump rate
    ...
    M4_WRITE_DBG(". exit ReadMatlvfourlvlepObj")

  end subroutine ReadMatlvfourlvlepObj

!----------------------------------------------------------------------

  subroutine InitializeMatlvfourlvlep

    integer :: err
    real (kind=8) :: conv1, conv2, g32, g30, g21, g10, ninit, pfac
    real(kind=8) :: x1, x2, re, im, tipangle !only for Superfluorescence test

    M4_MODLOOP_DECL({MATLVFOURLVLEP},mat) 
    M4_REGLOOP_DECL(reg,p,i,j,k,w(3))

    M4_WRITE_DBG(". enter InitializeMatlvfourlvlep")
    M4_MODLOOP_EXPR({MATLVFOURLVLEP},mat,{
...
!       allocate(mat%lvtermbx(reg%numnodes),mat%lvtermby(reg%numnodes),mat%lvtermbz(reg%numnodes),stat=err)
       M4_ALLOC_ERROR(err,"InitializeMatlvfourlvlep")
...
! load from checkpoint file
       ...
       if ( load_state .and. detail_level .ge. 2 ) then
          ...
          read(UNITCHK) mat%lvtermax, mat%lvtermay, mat%lvtermaz
!          read(UNITCHK) mat%Pbx, mat%Pby, mat%Pbz
!          read(UNITCHK) mat%lvtermbx, mat%lvtermby, mat%lvtermbz
          read(UNITCHK) mat%N0, mat%N1, mat%N2, mat%N3

       end if

       M4_IFELSE_DBG({call EchoMatlvfourlvlepObj(mat)},{call DisplayMatlvfourlvlepObj(mat)})

    })
    M4_WRITE_DBG(". exit InitializeMatlvfourlvlep")

  end subroutine InitializeMatlvfourlvlep

!----------------------------------------------------------------------

  subroutine FinalizeMatlvfourlvlep

    M4_MODLOOP_DECL({MATLVFOURLVLEP},mat)
    M4_REGLOOP_DECL(reg,p,i,j,k,w(3))

    M4_WRITE_DBG(". enter FinalizeMatlvfourlvlep")
    M4_MODLOOP_EXPR({MATLVFOURLVLEP},mat,{
...
! save to checkpoint file
       ...
       if ( save_state .and. detail_level .ge. 2 ) then
          ...
          write(UNITCHK) mat%lvtermax, mat%lvtermay, mat%lvtermaz
!          write(UNITCHK) mat%Pbx, mat%Pby, mat%Pbz
!          write(UNITCHK) mat%lvtermbx, mat%lvtermby, mat%lvtermbz
...
!       deallocate(mat%Pax, mat%Pay, mat%Paz, mat%Pbx, mat%Pby, mat%Pbz, mat%N0, mat%N1, mat%N2, mat%N3)
       deallocate(mat%Pax, mat%Pay, mat%Paz, mat%N0, mat%N1, mat%N2, mat%N3)

    })
    M4_WRITE_DBG(". exit FinalizeMatlvfourlvlep")

  end subroutine FinalizeMatlvfourlvlep

!----------------------------------------------------------------------

  subroutine StepHMatlvfourlvlep(ncyc)
...
!        pemb = mat%Pbx(p,m) * lEx + mat%Pby(p,m) * lEy + mat%Pbz(p,m) * lEz
  ...
  end subroutine StepHMatlvfourlvlep


!----------------------------------------------------------------------

  subroutine StepEMatlvfourlvlep(ncyc)
...
!           ( mat%Pbz(p,m) - mat%Pbz(p,n) ) )
  ...
  end subroutine StepEMatlvfourlvlep

!----------------------------------------------------------------------

  subroutine SumJEMatlvfourlvlep(mask, ncyc, sum, idx, mode)
...
!    sum(idx+2) = sum(idx+2) + sum2b !- sum1b
  ...
  end subroutine SumJEMatlvfourlvlep

!----------------------------------------------------------------------

  subroutine SumKHMatlvfourlvlep(mask, ncyc, sum, idx, mode)
  ...
  end subroutine SumKHMatlvfourlvlep

!----------------------------------------------------------------------

  subroutine DisplayMatlvfourlvlepObj(mat)
  ...
  end subroutine DisplayMatlvfourlvlepObj

!----------------------------------------------------------------------

   subroutine EchoMatlvfourlvlepObj(mat)

    type(T_MATLVFOURLVLEP) :: mat

    M4_WRITE_INFO({"--- matlvfourlvlep # ",&
  ...
  end subroutine EchoMatlvfourlvlepObj

  
!----------------------------------------------------------------------

end module matlvfourlvlep
