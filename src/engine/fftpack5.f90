...
!
  ...
  real cc(in1,l1,ido,2)
  real ch(in2,l1,2,ido)
  real chold1
  real chold2
  integer i
  integer k
...
!
  ...
  real cc(in1,l1,ido,3)
  real ch(in2,l1,3,ido)
  real ci2
  real ci3
  real cr2
  real cr3
...
!
  ...
  real cc(in1,l1,ido,5)
  real ch(in2,l1,5,ido)
  real chold1
  real chold2
  real ci2
  real ci3
  ...
  real ti4
  real ti5
  real, parameter :: ti11 =  0.9510565162951536E+00
  real, parameter :: ti12 =  0.5877852522924731E+00
  real tr2
  real tr3
  real tr4
  real tr5
  real, parameter :: tr11 =  0.3090169943749474E+00
  real, parameter :: tr12 = -0.8090169943749474E+00
  real wa(ido,4,2)
...
!
  ...
  real cc(in1,l1,ip,ido)
  real cc1(in1,lid,ip)
  real ch(in2,l1,ido,ip)
  real ch1(in2,lid,ip)
  real chold1
  real chold2
  integer i
...
!    transform is most efficient when N is a product of small primes.
!
!    Input, integer INC, the increment between the locations, in array C, 
!    of two consecutive elements within the sequence to be transformed.
!
!    Input/output, complex C(LENC) containing the sequence to be transformed.
!
!    Input, integer LENC, the dimension of the C array.  LENC must be at least 
!    INC*(N-1) + 1.
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with 
!    a call to CFFT1I before the first call to routine CFFT1F or CFFT1B 
!    for a given transform length N.  WSAVE's contents may be re-used for 
...
!
!    Workspace, real WORK(LENWRK).
!
!    Input, integer LENWRK, the dimension of the WORK array.  LENWRK must be at
!    least 2*N.
!
...
!
  ...
  if ( lensav < 2 * n + int ( log ( real ( n ) ) ) + 4 ) then
    ier = 2
...
!    transform is most efficient when N is a product of small primes.
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must 
!    be at least 2*N + INT(LOG(REAL(N))) + 4.
!
...
!
!    Input, integer L, the number of elements to be transformed in the 
!    first dimension of the two-dimensional complex array C.  The value 
!    of L must be less than or equal to that of LDIM.  The transform is
!    most efficient when L is a product of small primes.
!
!    Input, integer M, the number of elements to be transformed in the
!    second dimension of the two-dimensional complex array C.  The transform
!    is most efficient when M is a product of small primes. 
!
!    Input/output, complex C(LDIM,M), on intput, the array of two dimensions
!    containing the (L,M) subarray to be transformed.  On output, the
!    transformed data.
!
!    Input, real WSAVE(LENSAV). WSAVE's contents must be initialized with
...
!
!    Input, integer LENSAV, the dimension of the WSAVE array. LENSAV must be
!    at least 2*(L+M) + INT(LOG(REAL(L))) + INT(LOG(REAL(M))) + 8. 
!
!    Workspace, real WORK(LENWRK). 
!
!    Input, integer LENWRK, the dimension of the WORK array.  LENWRK must 
!    be at least 2*L*M. 
!
...
!
  iw = 2 * l + int ( log ( real ( l ) ) * log ( 2.0E+00 ) ) + 3

  call cfftmb ( l, 1, m, ldim, c, (l-1)+ldim*(m-1) +1, &
    wsave(iw), 2*m + int(log(real(m))) + 4, work, 2*l*m, ier1 )
...
!
  ...
  call cfftmb ( m, ldim, l, 1, c, (m-1)*ldim + l, wsave(iw), &
    2*l + int(log(real(l))) + 4, work, 2*m*l, ier1 )
...
!
!    Input, integer L, the number of elements to be transformed in the first 
!    dimension of the two-dimensional complex array C.  The value of L must 
!    be less than or equal to that of LDIM.  The transform is most efficient 
!    when L is a product of small primes. 
!
!    Input, integer M, the number of elements to be transformed in the 
!    second dimension of the two-dimensional complex array C.  The transform
!    is most efficient when M is a product of small primes. 
!
!    Input/output, complex C(LDIM,M), on input, the array of two dimensions
!    containing the (L,M) subarray to be transformed.  On output, the
!    transformed data.
!
!    Input, real WSAVE(LENSAV). WSAVE's contents must be initialized with a 
!    call to CFFT2I before the first call to routine CFFT2F or 
!    CFFT2B with transform lengths L and M.  WSAVE's contents may be re-used 
...
!
  ...
  call cfftmf ( l, 1, m, ldim, c, (l-1) + ldim*(m-1) +1, wsave(iw), &
    2*m + int(log(real(m))) + 4, work, 2*l*m, ier1 )
...
!
!    Input, integer L, the number of elements to be transformed in the 
!    first dimension.  The transform is most efficient when L is a product
!    of small primes.
!
!    Input, integer M, the number of elements to be transformed in the 
!    second dimension.  The transform is most efficient when M is a product
!    of small primes. 
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must 
!    be at least 2*(L+M) + INT(LOG(REAL(L))) + INT(LOG(REAL(M))) + 8.
!
!    Output, real WSAVE(LENSAV), contains the prime factors of L and M, and 
!    also certain trigonometric values which will be used in routines 
!    CFFT2B or CFFT2F. 
!
!    Output, integer IER, error flag.
!    0, successful exit;
...
!
  ...
  call cfftmi ( m, wsave(2*l+int(log(real(l))*log( 2.0E+00 )) + 3), &
...
!  Parameters:
!
!    Input, integer LOT, the number of sequences to be transformed within
!    array C. 
!
!    Input, integer JUMP, the increment between the locations, in array C,
!    of the first elements of two consecutive sequences to be transformed. 
!
!    Input, integer N, the length of each sequence to be transformed.  The
!    transform is most efficient when N is a product of small primes.
!
!    Input, integer INC, the increment between the locations, in array C,
!    of two consecutive elements within the same sequence to be transformed. 
!
!    Input/output, complex C(LENC), an array containing LOT sequences,
...
!    On output, C contains the transformed sequences.
!
!    Input, integer LENC, the dimension of the C array.  LENC must be at least
!    (LOT-1)*JUMP + INC*(N-1) + 1. 
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with 
!    a call to CFFTMI before the first call to routine CFFTMF 
...
!    of two consecutive elements within the same sequence to be transformed.
!
!    Input/output, complex C(LENC), array containing LOT sequences, each 
!    having length N, to be transformed.  C can have any number of dimensions,
!    but the total number of locations must be at least LENC.
!
!    Input, integer LENC, the dimension of the C array.  LENC must be at least
!    (LOT-1)*JUMP + INC*(N-1) + 1. 
!
...
!    Workspace, real WORK(LENWRK). 
!
!    Input, integer LENWRK, the dimension of the WORK array.  LENWRK must be
!    at least 2*LOT*N. 
!
!    Output, integer IER, error flag.
!    0 successful exit;
...
!
  ...
  real cc(2,in1,l1,ido,2)
  real ch(2,in2,l1,2,ido)
  real chold1
  real chold2
  integer i
  integer im1
...
!
  ...
  real cc(2,in1,l1,ido,3)
  real ch(2,in2,l1,3,ido)
  real ci2
  real ci3
  real cr2
  real cr3
...
!
  ...
  real cc(2,in1,l1,ido,4)
  real ch(2,in2,l1,4,ido)
  real ci2
  real ci3
  real ci4
  real cr2
...
!
  ...
  real cc(2,in1,l1,ido,5)
  real ch(2,in2,l1,5,ido)
  real chold1
  real chold2
  real ci2
  real ci3
...
!
  ...
  real cc(2,in1,l1,ip,ido)
  real cc1(2,in1,lid,ip)
  real ch(2,in2,l1,ido,ip)
  real ch1(2,in2,lid,ip)
  real chold1
  real chold2
  integer i
  integer idlj
...
!  Parameters:
!
!    Input, integer N, the number of elements to be transformed in the 
!    sequence.  The transform is most efficient when N is a 
!    product of small primes.
!
!    Input, integer INC, the increment between the locations, in array R,
!    of two consecutive elements within the sequence.
!
!    Input/output, real R(LENR); on input, containing the sequence to be 
!    transformed, and on output, containing the transformed sequence.
!
!    Input, integer LENR, the dimension of the R array.  LENR must be at least
!    INC*(N-1)+ 1.
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized 
!    with a call to COSQ1I before the first call to routine COSQ1F or 
!    COSQ1B for a given transform length N.  WSAVE's contents may be 
...
!
!    Output, real WSAVE(LENSAV), containing the prime factors of N and
!    also containing certain trigonometric values which will be used 
...
!    array R.
!
!    Input, integer JUMP, the increment between the locations, in array R,
!    of the first elements of two consecutive sequences to be transformed.
!
!    Input, integer N, the length of each sequence to be transformed.  The 
!    transform is most efficient when N is a product of small primes.
...
!
!    Input/output, real R(LENR), array containing LOT sequences, each having
!    length N.  R can have any number of dimensions, but the total number 
!    of locations must be at least LENR.  On input, R contains the data
...
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with
!    a call to COSQMI before the first call to routine COSQMF or COSQMB 
!    for a given transform length N.  WSAVE's contents may be re-used for 
...
!    of two consecutive elements within the sequence.
!
!    Input/output, real R(LENR), containing the sequence to be transformed.
!
!    Input, integer LENR, the dimension of the R array.  LENR must be at least
!    INC*(N-1)+ 1.
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with
!    a call to COST1I before the first call to routine COST1F or COST1B 
!    for a given transform length N.  WSAVE's contents may be re-used 
...
!
!    Input, integer N, the length of the sequence to be transformed.  The 
!    transform is most efficient when N-1 is a product of small primes.
!
!    Input, integer LENSAV, dimension of WSAVE array.  LENSAV must be at 
!    least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, real WSAVE(LENSAV), containing the prime factors of N and 
!    also containing certain trigonometric values which will be used in
...
!    of two consecutive elements within the same sequence.
!
!    Input/output, real R(LENR), array containing LOT sequences, each 
!    having length N.  On input, the data to be transformed; on output,
...
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with
!    a call to COSTMI before the first call to routine COSTMF or COSTMB
!    for a given transform length N.  WSAVE's contents may be re-used for
!    subsequent calls to COSTMF and COSTMB with the same N.
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must 
!    be at least 2*N + INT(LOG(REAL(N))) + 4.
!
...
!
  ...
  double precision cc(in1,ido,2,l1)
  double precision ch(in2,ido,l1,2)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  double precision cc(in1,ido,3,l1)
  double precision ch(in2,ido,l1,3)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  double precision cc(in1,ido,4,l1)
  double precision ch(in2,ido,l1,4)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  double precision arg
  double precision cc(in1,ido,5,l1)
  double precision ch(in2,ido,l1,5)
  integer i
  integer ic
  integer idp2
  integer k
...
!
  ...
  double precision arg
  double precision c1(in1,ido,l1,ip)
  double precision c2(in1,idl1,ip)
  double precision cc(in1,ido,ip,l1)
  double precision ch(in2,ido,l1,ip)
  double precision ch2(in2,idl1,ip)
...
!    of two consecutive elements within the sequence.
!
!    Input/output, double precision R(LENR); on input, containing the sequence to be 
!    transformed, and on output, containing the transformed sequence.
!
!    Input, integer LENR, the dimension of the R array.  LENR must be at least
!    INC*(N-1)+ 1.
!
!    Input, double precision WSAVE(LENSAV).  WSAVE's contents must be initialized 
!    with a call to DCOSQ1I before the first call to routine DCOSQ1F or 
!    DCOSQ1B for a given transform length N.  WSAVE's contents may be 
!    re-used for subsequent calls to DCOSQ1F and DCOSQ1B with the same N.
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must 
!    be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Workspace, double precision WORK(LENWRK).
!
!    Input, integer LENWRK, the dimension of the WORK array.  LENWRK must be at 
!    least N.
!
...
!
  ...
  if ( lensav < 2 * n + int ( log ( dble ( n ) ) ) + 4 ) then
    ier = 2
...
!    be at least 2*N + INT(LOG(REAL(N))) + 4.
!
!    Output, double precision WSAVE(LENSAV), containing the prime factors of N and
!    also containing certain trigonometric values which will be used 
...
!
  ...
  double precision c(in,*)
  double precision ch(*)
  double precision fac(15)
...
!    of two consecutive elements within the sequence. 
!
!    Input/output, double precision R(LENR), on input, the data to be transformed, and
!    on output, the transformed data.
!
!    Input, integer LENR, the dimension of the R array.  LENR must be at 
!    least INC*(N-1) + 1. 
!
!    Input, double precision WSAVE(LENSAV).  WSAVE's contents must be initialized 
!    with a call to DFFT1I before the first call to routine
!    DFFT1F or DFFT1B for a given transform length N.
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must
!    be at least N + INT(LOG(REAL(N))) + 4. 
!
!    Workspace, double precision WORK(LENWRK).
!
!    Input, integer LENWRK, the dimension of the WORK array.  LENWRK must be
...
!
  ...
  if ( lensav < n + int ( log ( real ( n ) ) ) + 4 ) then
    ier = 2
...
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must 
!    be at least N/2 + N + INT(LOG(REAL(N))) + 4.
!
!    Workspace, real ( kind = 8 ) WORK(LENWRK).
!
!    Input, integer LENWRK, the dimension of the WORK array.  Must be at least
!    2*N+2.
!
!    Output, integer IER, error flag.
!    0, successful exit;
...
!
  ...
  if ( lensav < n / 2 + n + int ( log ( real ( n ) ) ) + 4 ) then
    ier = 2
...
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must be
!    at least N/2 + N + INT(LOG(REAL(N))) + 4.
!
!    Output, real ( kind = 8 ) WSAVE(LENSAV), containing the prime factors
...
!
  ...
  real cc(in1,ido,2,l1)
  real ch(in2,ido,l1,2)
  integer i
  integer ic
  integer idp2
  integer im1
...
!
  ...
  real arg
  real cc(in1,ido,3,l1)
  real ch(in2,ido,l1,3)
  integer i
  integer ic
  integer idp2
  integer im1
...
!
  ...
  real cc(in1,ido,4,l1)
  real ch(in2,ido,l1,4)
  integer i
  integer ic
  integer idp2
  integer im1
...
!
  ...
  real arg
  real cc(in1,ido,5,l1)
  real ch(in2,ido,l1,5)
  integer i
  integer ic
  integer idp2
  integer im1
...
!
  ...
  real ar2h
  real arg
  real c1(in1,ido,l1,ip)
  real c2(in1,idl1,ip)
  real cc(in1,ido,ip,l1)
  real ch(in2,ido,l1,ip)
  real ch2(in2,idl1,ip)
  real dc2
  real dcp
  real ds2
  real dsp
...
!
  ...
  real cc(in1,ido,l1,2)
  real ch(in2,ido,2,l1)
  integer i
  integer ic
  integer idp2
  integer im1
...
!
  ...
  real arg
  real cc(in1,ido,l1,3)
  real ch(in2,ido,3,l1)
  integer i
  integer ic
  integer idp2
  integer im1
...
!
  ...
  real cc(in1,ido,l1,4)
  real ch(in2,ido,4,l1)
  real hsqt2
  integer i
  integer ic
  integer idp2
...
!
  ...
  real arg
  real cc(in1,ido,l1,5)
  real ch(in2,ido,5,l1)
  integer i
  integer ic
  integer idp2
  integer im1
...
!    is needed.
!
!    Output, real WA(N), trigonometric information.
!
!    Output, real FAC(15), factorization information.  FAC(1) is N, 
!    FAC(2) is NF, the number of factors, and FAC(3:NF+2) are the factors.
!
  implicit none
...
!    of two consecutive elements within the sequence. 
!
!    Input/output, real R(LENR), on input, the data to be transformed, and
!    on output, the transformed data.
!
!    Input, integer LENR, the dimension of the R array.  LENR must be at 
!    least INC*(N-1) + 1. 
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized 
!    with a call to RFFT1I before the first call to routine
...
!    of two consecutive elements within the sequence. 
!
!    Input/output, real R(LENR), on input, contains the sequence to be
...
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with
!    a call to RFFT1I before the first call to routine RFFT1F
...
!  Parameters:
!
!    Input, integer LDIM, the first dimension of the two-dimensional real 
!    array R, which must be at least 2*(L/2+1).
!
!    Input, integer L, the number of elements to be transformed in the first
!    dimension of the two-dimensional real array R.  The value of L must be 
!    less than or equal to that of LDIM.  The transform is most efficient 
!    when L is a product of small primes.
!
!    Input, integer M, the number of elements to be transformed in the 
!    second dimension of the two-dimensional real array R.  The transform
!    is most efficient when M is a product of small primes.
!
!    Input/output, real R(LDIM,M), the real array of two dimensions.  On input,
!    R contains the L/2+1-by-M complex subarray of spectral coefficients,
!    on output, the physical coefficients.
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with 
!    a call to RFFT2I before the first call to routine RFFT2F or RFFT2B
!    with lengths L and M.  WSAVE's contents may be re-used for subsequent
!    calls to RFFT2F and RFFT2B with the same transform lengths L and M. 
!
!    Input, integer LENSAV, the number of elements in the WSAVE array.
!    LENSAV must be at least L + M + INT(LOG(REAL(L))) + INT(LOG(REAL(M))) + 8.
!
!    Workspace, real WORK(LENWRK).  WORK provides workspace, and its 
!    contents need not be saved between calls to routines RFFT2B and RFFT2F.
!
!    Input, integer  LENWRK, the number of elements in the WORK array. 
!    LENWRK must be at least LDIM*M.
!
!    Output, integer IER, the error flag.
!    0, successful exit;
!    2, input parameter LENSAV not big enough;
...
!
  call cfftmb ( l/2+1, 1, m, ldim/2, r, m*ldim/2, &
    wsave(l+int(log(real(l)))+5), 2*m+int(log(real(m)))+4, work, &
    2*(l/2+1)*m, ier1 )
...
!    when L is a product of small primes.
!
!    Input, integer M, the number of elements to be transformed in the second 
!    dimension of the two-dimensional real array R.  The transform is most
!    efficient when M is a product of small primes. 
!
!    Input/output, real R(LDIM,M), the real array of two dimensions.
!    On input, containing the L-by-M physical data to be transformed. 
!    On output, the spectral coefficients.
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with 
!    a call to RFFT2I before the first call to routine RFFT2F or RFFT2B 
!    with lengths L and M.  WSAVE's contents may be re-used for subsequent 
!    calls to RFFT2F and RFFT2B with the same transform lengths. 
!
!    Input, integer LENSAV, the number of elements in the WSAVE array.  LENSAV
!    must be at least L + M + INT(LOG(REAL(L))) + INT(LOG(REAL(M))) + 8. 
!
!    Workspace, real WORK(LENWRK), provides workspace, and its contents 
!    need not be saved between calls to routines RFFT2F and RFFT2B. 
!
!    Input, integer LENWRK, the number of elements in the WORK array.  LENWRK
!    must be at least LDIM*M.
!
!    Output, integer IER, the error flag.
!    0, successful exit;
!    2, input parameter LENSAV not big enough;
...
!
!    Input, integer L, the number of elements to be transformed in the first
!    dimension.  The transform is most efficient when L is a product of small
!    primes.
!
!    Input, integer M, the number of elements to be transformed in the second
!    dimension.  The transform is most efficient when M is a product of small
!    primes. 
!
!    Input, integer LENSAV, the number of elements in the WSAVE array. 
!    LENSAV must be at least L + M + INT(LOG(REAL(L))) + INT(LOG(REAL(M))) + 8.
!
!    Output, real WSAVE(LENSAV), containing the prime factors of L and M, 
!    and also containing certain trigonometric values which will be used in
!    routines RFFT2B or RFFT2F.
!
!    Output, integer IER, error flag.
!    0, successful exit;
!    2, input parameter LENSAV not big enough;
...
!    Output, real WA(N), trigonometric information.
!
!    Output, real FAC(15), factorization information.  
!    FAC(1) is N, FAC(2) is NF, the number of factors, and FAC(3:NF+2) are the 
!    factors.
!
  implicit none
...
!    of two consecutive elements within the same sequence.
!
!    Input/output, real R(LENR), real array containing LOT sequences, 
!    each having length N.  R can have any number of dimensions, but the
!    total number of locations must be at least LENR.  On input, the
...
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized 
!    with a call to RFFTMI before the first call to routine RFFTMF or 
!    RFFTMB for a given transform length N.  
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must 
!    be at least N + INT(LOG(REAL(N))) + 4.
!
...
!    be at least N + INT(LOG(REAL(N))) + 4.
!
!    Output, real WSAVE(LENSAV), work array containing the prime factors 
!    of N and also containing certain trigonometric 
...
!    of two consecutive elements within the sequence.
!
!    Input/output, real R(LENR), on input, the sequence to be transformed.
...
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with 
!    a call to SINQ1I before the first call to routine SINQ1F or SINQ1B 
!    for a given transform length N.  WSAVE's contents may be re-used for
!    subsequent calls to SINQ1F and SINQ1B with the same N.
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must be
!    at least 2*N + INT(LOG(REAL(N))) + 4.
!
...
!    of two consecutive elements within the same sequence.
!
!    Input/output, real R(LENR), containing LOT sequences, each having 
!    length N.  R can have any number of dimensions, but the total number 
!    of locations must be at least LENR.  On input, R contains the data
!    to be transformed, and on output the transformed data.
!
...
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with
!    a call to SINQMI before the first call to routine SINQMF or SINQMB for
!    a given transform length N.  WSAVE's contents may be re-used for 
!    subsequent calls to SINQMF and SINQMB with the same N.
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must be
!    at least 2*N + INT(LOG(REAL(N))) + 4.
!
...
!
!    Input/output, real R(LENR), on input, contains the sequence to be 
!    transformed, and on output, the transformed sequence.
!
!    Input, integer LENR, the dimension of the R array.  LENR must be at least
!    INC*(N-1)+ 1.
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with 
!    a call to SINT1I before the first call to routine SINT1F or SINT1B 
!    for a given transform length N.  WSAVE's contents may be re-used for
!    subsequent calls to SINT1F and SINT1B with the same N.
!
!    Input, integer LENSAV, the dimension of the WSAVE array.  LENSAV must 
!    be at least N/2 + N + INT(LOG(REAL(N))) + 4.
!
...
!
!    Input, real WSAVE(LENSAV).  WSAVE's contents must be initialized with 
!    a call to SINTMI before the first call to routine SINTMF or SINTMB 
!    for a given transform length N.  WSAVE's contents may be re-used 
...
!
!    Input, integer INFO, an error code.  When a single invalid parameter in 
!    the parameter list of the calling routine has been detected, INFO is 
!    the position of that parameter.  In the case when an illegal combination
!    of LOT, JUMP, N, and INC has been detected, the calling subprogram calls 
!    XERFFT with INFO = -1.
!
  implicit none
