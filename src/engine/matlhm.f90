    ...
    M4_MODOBJ_GETREG(mat,reg)
       ...
       if ( mask(i,j,k)  ) then
          ...
          if ( mat%order .eq. 1 ) then ! 1. order equation
             ...
             sum = sum + ( &
M4_IFELSE_TM({    M4_VOLHX(i,j,k) * w(4) * real(Hx(i,j,k)) * real(mat%Kx(p,1)) +},{0. +}) &
M4_IFELSE_TM({    M4_VOLHY(i,j,k) * w(5) * real(Hy(i,j,k)) * real(mat%Ky(p,1)) +},{0. +}) &
M4_IFELSE_TE({    M4_VOLHZ(i,j,k) * w(6) * real(Hz(i,j,k)) * real(mat%Kz(p,1))  },{0.  }) &
                  )

          else

              sum = sum + ( & 
M4_IFELSE_TM({    M4_VOLHX(i,j,k) * w(4) * real(Hx(i,j,k)) * real( mat%Kx(p,m) - mat%Kx(p,n) ) / DT +},{0. +}) &
M4_IFELSE_TM({    M4_VOLHY(i,j,k) * w(5) * real(Hy(i,j,k)) * real( mat%Ky(p,m) - mat%Ky(p,n) ) / DT +},{0. +}) &
M4_IFELSE_TE({    M4_VOLHZ(i,j,k) * w(6) * real(Hz(i,j,k)) * real( mat%Kz(p,m) - mat%Kz(p,n) ) / DT  },{0.  }) &
                   )
