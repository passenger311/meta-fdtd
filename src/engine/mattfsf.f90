...
!----------------------------------------------------------------------
  ...
  subroutine InitializeMatTfsf
    ...
    type(T_REG) :: reg
    integer :: i,j,k
    real(kind=8) :: r, rk(3), ksq, in(3), mdelay, pvel
    integer :: err
    ...
    in(1) = sqrt( epsinvx(i,j,k) * M4_MUINVX(i,j,k) )  ! assumed to be homogeneous over all faces.
    in(2) = sqrt( epsinvy(i,j,k) * M4_MUINVY(i,j,k) )
    in(3) = sqrt( epsinvz(i,j,k) * M4_MUINVZ(i,j,k) )
    
    mat%invmc(1) = epsinvx(i,j,k)
    mat%invmc(2) = epsinvy(i,j,k)
    mat%invmc(3) = epsinvz(i,j,k)

    mat%invmc(4) = M4_MUINVX(i,j,k)
    ...
    mat%cdelay(1) = in(1)*.5*SX*mat%kinc(1) /  ( pvel * DT )
    mat%cdelay(2) = in(2)*.5*SY*mat%kinc(2) /  ( pvel * DT )
    mat%cdelay(3) = in(3)*.5*SZ*mat%kinc(3) /  ( pvel * DT )
...
!                 dd = d - di
                 ...
                 !     dd * mat%signale(mod(n-di-1,mat%maxdelay))
