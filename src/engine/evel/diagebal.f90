...
module diagebal
  ...
  real(kind=8), dimension(3) :: ux, uy, uz, uxx, uyy, uzz
  real(kind=8), dimension(3) :: dudtr, dudtrr
  real(kind=8), dimension(3) :: sumdudtr, sumdudtrr
  real(kind=8), dimension(3) :: jedispr, jedisprr
  })
...
!----------------------------------------------------------------------
  ...
  subroutine StepEDiagEBal(ncyc)
    ...
    M4_MODLOOP_EXPR({DIAGEBAL},diag,{
       ...
       diag%en(m) = diag%en(m) + u

       diag%ux(m) =  diag%ux(m) + u*i 
       diag%uy(m) =  diag%uy(m) + u*j
       diag%uz(m) =  diag%uz(m) + u*k 
       diag%uxx(m) =  diag%uxx(m) + u*i*i 
       diag%uyy(m) =  diag%uyy(m) + u*j*j 
       diag%uzz(m) =  diag%uzz(m) + u*k*k 
...
!       diag%jekh = .5 * ( diag%q1 + diag%q2(mo) + diag%q3 + diag%q4 ) 
       ...
       diag%dudtr(1) = .5/DT * ( diag%ux(m) - diag%ux(moo) )
       diag%dudtr(2) = .5/DT * ( diag%uy(m) - diag%uy(moo) )
       diag%dudtr(3) = .5/DT * ( diag%uz(m) - diag%uz(moo) )
       diag%dudtrr(1) = .5/DT * ( diag%uxx(m) - diag%uxx(moo) )
       diag%dudtrr(2) = .5/DT * ( diag%uyy(m) - diag%uyy(moo) )
       diag%dudtrr(3) = .5/DT * ( diag%uzz(m) - diag%uzz(moo) )
       ...
       do i = 1,idx-1,NUMEBALCH
          ...
          diag%jedispr(1) = diag%je(i+3)
          diag%jedispr(2) = diag%je(i+4)
          diag%jedispr(3) = diag%je(i+5)
          diag%jedisprr(1) = diag%je(i+6)
          diag%jedisprr(2) = diag%je(i+7)
          diag%jedisprr(3) = diag%je(i+8)
       enddo

       diag%dudt = diag%dudt + diag%jedisp + diag%khdisp
       diag%dudtr = diag%dudtr + diag%jedispr 
       diag%dudtrr = diag%dudtrr + diag%jedisprr 

       diag%ds   = diag%dsx + diag%dsy + diag%dsz 
       diag%res = - ( diag%dudt + diag%ds + diag%jeabs + diag%khabs )
       
       diag%sumdudt = diag%sumdudt + DT * diag%dudt 
       diag%sumdudtr = diag%sumdudtr + DT * diag%dudtr
       diag%sumdudtrr = diag%sumdudtrr + DT * diag%dudtrr
